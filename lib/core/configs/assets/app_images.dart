class AppImages {
  static const String baseUrl = 'assets/images/';
  static const String format = '.png';

  static const String userAvatar = '${baseUrl}user_avatar$format';
  static const String saleAdvert = '${baseUrl}sale_ad$format';

  static const String swidLogo = '${baseUrl}swid_logo$format';
  static const String complexLogo = '${baseUrl}complex_logo$format';
  static const String poweredBy = '${baseUrl}powered_by$format';
  static const String iomLogo = "${baseUrl}logo_iom$format";

  static const String rusTiliKursi = '${baseUrl}rustili_kursi$format';
  static const String grandHat = '${baseUrl}grand_hat$format';
  static const String singleCourse = "${baseUrl}single_course_pic$format";

  static const String thumbnail = '${baseUrl}thumbnail_vid$format';
  static const String payMe = '${baseUrl}payme$format';
  static const String video = '${baseUrl}video$format';
  static const String bookLamp = '${baseUrl}book_lamp$format';

  static const String defaultProfilePic = '${baseUrl}default_pic$format';
  static const String searchNotFound = '${baseUrl}search_not_found$format';

  static const String courseGlobe = '${baseUrl}course_globe$format';
  static const String courseBook = '${baseUrl}course_books$format';
  static const String courseTest = '${baseUrl}course_test$format';

  static const String sertifikatPagePaper = "${baseUrl}complate_paper$format";
  static const String certificate = '${baseUrl}real_certificate$format';
}
