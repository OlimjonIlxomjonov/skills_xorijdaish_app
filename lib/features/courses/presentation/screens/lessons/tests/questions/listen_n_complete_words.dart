...
class ListenNCompleteWordsPage extends StatefulWidget {
  final int courseId;
  final int lessonId;
  final int questionId;
  final List<int> selectedWordsId;

  const ListenNCompleteWordsPage({
    super.key,
  ...
  @override
  State<ListenNCompleteWordsPage> createState() =>
      _ListenNCompleteWordsPageState();
}

class _ListenNCompleteWordsPageState extends State<ListenNCompleteWordsPage> {
  ...
  void initState() {
    super.initState();
    context.read<ListenNCompleteWordsBloc>().add(
      ListenNCompleteWordsEvent(
        widget.courseId,
        widget.lessonId,
        widget.questionId,
      ),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: BlocBuilder<ListenNCompleteWordsBloc, ListenNCompleteWordsState>(
        builder: (context, state) {
          if (state is ListenNCompleteWordsLoading) {
            return CircularProgressIndicator();
          } else if (state is ListenNCompleteWordsLoaded) {
            return Column(
              children: [
