...
class FinalListeNSelectPairsPage extends StatefulWidget {
  final int courseId;
  final int questionId;
  final String testType;
  final VoidCallback? onTestComplete;

  const FinalListeNSelectPairsPage({
    super.key,
  ...
  @override
  State<FinalListeNSelectPairsPage> createState() =>
      _FinalListeNSelectPairsPageState();
}

class _FinalListeNSelectPairsPageState
    extends State<FinalListeNSelectPairsPage> {
  ...
  void _onAnswerSuccess() {
    ...
    final totalPairs =
        context.read<FetchFinalListenNSelectPairsBloc>().state
                is FetchFinalListenNSelectPairsLoaded
            ? (context.read<FetchFinalListenNSelectPairsBloc>().state
                    as FetchFinalListenNSelectPairsLoaded)
                .entity
                .options
                .length
            : 0;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: BlocListener<FinalSelectPairsBloc, FinalSelectPairsState>(
        ...
        child: BlocBuilder<
          FetchFinalListenNSelectPairsBloc,
          FetchFinalListenNSelectPairsState
        >(
          builder: (context, state) {
            if (state is FetchFinalListenNSelectPairsLoading) {
              return Center(child: CircularProgressIndicator());
            } else if (state is FetchFinalListenNSelectPairsLoaded) {
